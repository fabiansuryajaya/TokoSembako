...
switch ($method) {
    case 'read':
        ...
        $sql = "SELECT dp.id_detail_pembelian, dp.id_pembelian, dp.id_produk, p.nama_product, s.nama_satuan, sup.nama_supplier, dp.jumlah_pembelian, dp.harga_pembelian, dp.harga_jual, dp.created_at
        ...
        if (isset($query_data['from_date']) && isset($query_data['to_date'])) {
            ...
            $sql .= " AND DATE(dp.created_at) BETWEEN '$from_date' AND '$to_date'";
        }
        ...
        if ($action === 'detail') {
            ...
            if (isset($query_data['id_detail'])) {
                ...
                $sql = "SELECT dp.*, p.nama_product, s.nama_satuan, sup.nama_supplier, dp.harga_jual, p.stok_product, dp.tax, dp.created_at
                        FROM detail_pembelian dp
                        JOIN product p ON dp.id_produk = p.id_product
                        JOIN satuan s ON p.id_satuan = s.id_satuan
                        JOIN supplier sup ON p.id_supplier = sup.id_supplier
    ...
    case 'create':
        ...
        // insert detail
        $sql = "INSERT INTO detail_pembelian (id_pembelian, id_produk, jumlah_pembelian, harga_pembelian, tax, harga_jual) VALUES ($id_pembelian, $product_id, $restock_quantity, $harga_beli, $tax, $harga_jual)";
        if ($conn->query($sql) === FALSE) {
            http_response_code(500);
        ...
        // Update stok product
        $sql = "UPDATE product SET stok_product = stok_product + $restock_quantity, harga_beli_product = $harga_beli WHERE id_product = $product_id";
        if ($conn->query($sql) === FALSE) {
            http_response_code(500);
